...
export async function list(context: RequestContext): Promise<ListResult> {
  ...
  if (groupFilters.length > 0) {
    const groupItemIds = await processGroupFilters(groupFilters, 'server');
  ...
  const [items, total] = await Promise.all([
    ...
    prisma.server.count({ where: finalWhere }),
  ]);

  // Remove passwords from response
  const itemsWithoutPassword = items.map(removePassword);

  return {
    data: itemsWithoutPassword,
    pagination: {
      page,
      limit,
      total,
