...
export function ServiceGroups({ serviceId, groupsMap }: ServiceGroupsProps) {
  const { hasPermission } = useAuth();
  const navigate = useNavigate();
  
  const { data: groupIds } = useQuery({
    queryKey: ['groups', 'service', serviceId],
    queryFn: () => getGroupsByItem('service', serviceId),
    enabled: hasPermission('groups:view') && !!serviceId,
    staleTime: 5 * 60 * 1000,
  });
