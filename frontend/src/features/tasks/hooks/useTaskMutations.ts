...
export function useUpdateTaskStatus() {
  const queryClient = useQueryClient();
  return useMutation<Task, Error, { id: number; status: TaskStatus; testingSkipped?: boolean; testingSkipReason?: string }>({
    mutationFn: ({ id, status, testingSkipped, testingSkipReason }) =>
      updateTaskStatus(id, { status, testingSkipped, testingSkipReason }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
      queryClient.invalidateQueries({ queryKey: ['task', variables.id] });
